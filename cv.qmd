...
yearSincePhD <- round(as.numeric(
  difftime(Sys.Date(), as.Date("2015-06-12"),
           units = "days") / 365.25), 1)
...
assocprofs <- list(
  ...
  ##`Sean Cain` = getCites("fCfy7iwAAAAJ"),
  `Trevor Chong` = getCites("qLkvokIAAAAJ"),
  ## james courtney cannot find google scholar
  ...
  ## david moseley cannot find google scholar
...
compareplot.assoc <- ggplot(d2, aes(year, cites, group = Name, colour = Select, linewidth = Select)) +
  ...
  scale_y_continuous("Yearly Citations (Google Scholar)",
                     ...
                     trans = "log2") +
  theme_classic() +
  coord_cartesian(xlim = c(2013 - .2, cyear + .4), ylim = c(10, 4300), expand = FALSE) +
  theme(legend.position.inside = c(.8, .15), axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(caption = "Numbers show Joshua F Wiley's rank")


profs <- list(
  `Mark Bellgrove` = getCites("yb3omigAAAAJ"),
  `Adeel Razi` = getCites("VG3onZwAAAAJ"),
  `Sue Cotton` = getCites("VW_B2SoAAAAJ"),
  `Sean Drummond` = getCites("ow8CMQ4AAAAJ"),
  `Alex Fornito` = getCites("uceaXuYAAAAJ"),
  ##`Judith Gullifer` = getCites("8MdoxdsAAAAJ"),
  `Jennie Ponsford` = getCites("_wstNVwAAAAJ"),
  `Nicole Rinehart` = getCites("wLwcdSMAAAAJ"), 
  `Martin Sellbom` = getCites("t0jyZIUAAAAJ"),
  `Julie C Stout` = getCites("CFTw2MEAAAAJ"),
  `Shantha Rajaratnam` = getCites("Fah_71UAAAAJ"),
  `Nao Tsuchiya` = getCites("Or751n4AAAAJ"),
  `Antonio Verdejo-Garcia` = getCites("_Tb1r8IAAAAJ"),
...
d <- as.data.table(do.call(rbind, lapply(names(profs), function(v) {
  cbind(profs[[v]], Name = v, Select = ifelse(v == "Joshua F Wiley",
                                                        "Joshua F Wiley", "Level E (Prof)\nComparators"))
})))
d2 <- d[Type == "Yearly" & year > 2012]
d2[, Rank := paste0(rank(-cites, ties.method = "max"), "/", length(profs)), by = year]
sqdiff <- function(x, j = .5) abs((x - (sqrt(x) + j)^2) )

compareplot.profs <- ggplot(d2, aes(year, cites, group = Name, colour = Select, linewidth = Select)) +
  geom_line() +
  geom_label(aes(year, cites + sqdiff(cites, 1), label = Rank),
             data = d2[Name=="Joshua F Wiley"], show.legend = FALSE) +
  scale_colour_manual("Who", values = c("Joshua F Wiley" = "black", "Level E (Prof)\nComparators" = "grey60")) +
  scale_linewidth_manual("Who", values = c("Joshua F Wiley" = 2, "Level E (Prof)\nComparators" = .5)) +
  scale_x_continuous("Year", breaks = 2013:cyear,
                     labels = ifelse(
                       2013:cyear == 2015,
                       "2015\n(PhD Awarded)",
...
compareplot <- ggarrange(
  compareplot.assoc,
  compareplot.profs,
  ncol = 1, nrow = 2)

ggsave("compareplot.pdf", compareplot, width = 8, height = 12,
        device = "pdf")
...
## pubs[MonashCountsIF == 1, sum(IF, na.rm = TRUE), by = Year][Year >= 2020, mean(V1)]
...
pubcounts[, Year := factor(Year,
                           levels = pubyears,
                           labels = fifelse(pubyears == "2015", "2015\n(PhD)", fifelse(pubyears == "2020", "2020\n(Level C)", pubyears)))]
...
tmpcitecounts <- as.data.table(profs[["Joshua F Wiley"]])
tmpcitecounts[, Type := factor(Type, levels = c("Yearly", "Cumulative"))]
tmpcitecounts[, year := factor(year,
                           levels = pubyears[-1],
                           labels = fifelse(pubyears[-1] == "2015", "2015\n(PhD)", fifelse(pubyears[-1] == "2020", "2020\n(Level C)", pubyears[-1])))]

citecounts.graph <- ggplot(tmpcitecounts, aes(year, cites)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ Type, scales = "free_y") +
  theme_classic() +
  scale_y_continuous("Citations (Google Scholar)") +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
        axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x = element_blank()) 
...
>$14 million AUD in grant funding as a chief investigator; 
...
mc <-  cranDownloads(
      packages = "multilevelcoda",
      from = "2025-01-01", to = "2025-10-14")
...
sum(pkgdownloads$y2025$cranlogs.data$count)

x2025d <- as.data.table(pkgdownloads$y2025$cranlogs.data)[,
  .(YTD_Download = sum(count)), by = package]

if (FALSE) {
  x2025 <- lapply(1:31, function(i) {
   if (i < 10) {
    usedate <- sprintf("2025-03-0%d", i)
  } else {
    usedate <- sprintf("2025-03-%d", i)
  }
  packageRank(package = myPackages, date = usedate)
})
  saveRDSfst(x2025, file = "packageranks_2025_0301-0331.RDS")
} else {
  x2025 <- readRDSfst("packageranks_2025_0301-0331.RDS")
}

x2025b <- as.data.table(do.call(rbind, lapply(x2025, function(x) {
  cbind(x$package.data, TotalPkgs = length(x$freqtab))
})))

x2025b[, M := mean(percentile), by = package]
x2025b[!duplicated(package)][order(-M), .(package, M)]
x2025b[, package := factor(package, levels = unique(package[order(-M)]))]

x2025b[!duplicated(date), mean(TotalPkgs)]

x2025d <- as.data.table(pkgdownloads$y2025$cranlogs.data)[
  date %between% c("2025-03-01", "2025-03-31"),
  .(March_Downloads = sum(count)), by = package]

percplot <- ggplot(x2025b, aes(package, percentile / 100)) + 
  geom_bar(data = x2025d, aes(x = package, y = March_Downloads / max(March_Downloads)),
           stat = "identity", fill = "grey80", width = .5) +
  geom_boxplot() +
  geom_jitter(width = .2, height = 0, alpha = .5) +
  scale_y_continuous("Percentile Rank", labels = scales::percent,
    sec.axis = sec_axis(~ . * max(x2025d$March_Downloads), name = "Downloads")) +
  theme_classic() +
  geom_label(data = x2025b[!duplicated(package)],
             aes(x = package, y = .95, label = sprintf("M=%0.1f", M)),
             nudge_y = 0, size = 3) +
  labs(x = "",
       title = "R Package Ranks & Downloads (March 2025)",
       caption = "Boxplots show distribution of daily percentiles for March 2025 (left axis).\nBars show total downloads for March 2025 (right axis).\nM = mean percentile ranking for the month.")
print(percplot)

ggsave("march2025_packagedata.pdf", percplot, width = 9, height = 7,
        device = "pdf")

percplot + coord_cartesian(ylim = c(.80, 1), expand = FALSE)
...
`2010 - 12 June 2015` (PhD pass date) 
